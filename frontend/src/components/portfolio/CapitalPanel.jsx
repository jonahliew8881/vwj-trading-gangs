...
export default function CapitalPanel({ usdHkdRate, onRateChange }) {
  ...
  return (
    <div>
      ...
      {/* Total HKD */}
      <div style={{
        background: 'var(--cy-panel)', border: '1px solid var(--cy-border)',
        borderLeft: '3px solid var(--cy-green)',
...
function TxnRow({ txn: t, onEdit, onDelete }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '8px', fontWeight: 700, color: typeColor[t.type], letterSpacing: '0.5px' }}>
        {typeLabel[t.type]}
      </span>
      <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '8px', color: 'var(--cy-muted)' }}>
  ...
  return (
    <Modal title="Deposit / Withdraw" onClose={onClose}>
      ...
      <div style={{ marginTop: '7px' }}>
        ...
        <input style={inputStyle} placeholder="e.g. Monthly top-up" value={form.note} onChange={e => set('note', e.target.value)} />
      </div>
      {error && <div style={{ color: 'var(--cy-red)', fontSize: '10px', marginTop: '6px', fontFamily: "'Montserrat', sans-serif" }}>{error}</div>}
      <button onClick={handleSubmit} disabled={loading} style={{
        width: '100%', marginTop: '10px', padding: '8px',
        background: form.type === 'deposit' ? 'rgba(0,255,136,0.08)' : 'rgba(255,51,85,0.08)',
        border: `1px solid ${form.type === 'deposit' ? 'rgba(0,255,136,0.4)' : 'rgba(255,51,85,0.4)'}`,
        color: form.type === 'deposit' ? 'var(--cy-green)' : 'var(--cy-red)',
