...
export default function Journal() {
  ...
  function calcStats(data) {
    ...
    const totalPnL = data.reduce((s, t) => s + parseFloat(t.pnl), 0)

    // EV = winRate * avgWin - lossRate * avgLoss (in R)
    ...
    // Auto-update Kelly if 30+ trades
    if (data.length >= 30) {
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <StatMini
          val={stats.winRate !== null ? `${(stats.winRate * 100).toFixed(1)}%` : '—'}
          label="Win Rate"
          color={stats.winRate >= 0.5 ? 'var(--cy-green)' : 'var(--cy-red)'}
          sub={`${stats.count} trades`}
        />
        <StatMini
          val={stats.avgRR !== null ? `${stats.avgRR.toFixed(1)}x` : '—'}
          label="Avg R:R"
          color="var(--cy-accent)"
          sub="per trade"
        />
        <StatMini
          val={stats.ev !== null ? `${stats.ev >= 0 ? '+' : ''}${stats.ev.toFixed(2)}` : '—'}
          label="EV / Dollar"
          color={stats.ev >= 0 ? 'var(--cy-green)' : 'var(--cy-red)'}
          sub="expected value"
        />
        <StatMini
          val={stats.mtdPnL !== 0 ? `${stats.mtdPnL >= 0 ? '+' : ''}$${Math.abs(stats.mtdPnL).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : '$0'}
          label="Realized MTD"
          color={stats.mtdPnL >= 0 ? 'var(--cy-green)' : 'var(--cy-red)'}
          sub="this month"
        />
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: 'var(--cy-muted)' }}>
          Kelly Win Rate Source:
        </span>
        <span style={{
          fontFamily: "'Share Tech Mono', monospace", fontSize: '9px',
          color: stats.count >= 30 ? 'var(--cy-green)' : 'var(--cy-yellow)',
        }}>
          {stats.count >= 30
      ...
      {loading ? (
        ...
        <div style={{ padding: '20px', textAlign: 'center', color: 'var(--cy-muted)', fontFamily: "'Share Tech Mono', monospace", fontSize: '11px' }}>
          // No trades logged yet. Click + Log Trade to start.
        </div>
      ) : (
      ...
      {showModal && (
        <LogTradeModal
          ...
          onSuccess={() => { setShowModal(false); fetchTrades() }}
        />
      )}
    </div>
  )
...
function TradeRow({ trade: t }) {
  ...
  return (
    <div style={{
      background: 'var(--cy-grid)',
      borderLeft: `3px solid ${isWin ? 'var(--cy-green)' : 'var(--cy-red)'}`,
      padding: '7px 9px',
      display: 'grid',
      ...
      <div>
        <div>
          <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '12px', color: 'var(--cy-accent)', fontWeight: 600 }}>
            {t.ticker}
          </span>
          <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '8px', color: 'var(--cy-muted)', marginLeft: '6px' }}>
            Long · {t.quantity} · {t.account}
          </span>
        </div>
        <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '8px', color: 'var(--cy-muted)', marginTop: '2px' }}>
          ${t.entry_price} → ${t.exit_price}
          {t.strategy_used && ` · ${t.strategy_used}`}
        </div>
        {t.notes && (
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '8px', color: 'var(--cy-muted)', marginTop: '1px', fontStyle: 'italic' }}>
            {t.notes}
          </div>
        )}
      </div>
      <div style={{ textAlign: 'right' }}>
        <div style={{
          ...
          color: pnl >= 0 ? 'var(--cy-green)' : 'var(--cy-red)',
        }}>
          {pnl >= 0 ? '+' : ''}${Math.abs(pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}
        </div>
        <div style={{
          fontFamily: "'Share Tech Mono', monospace", fontSize: '12px',
          color: parseFloat(rr) >= 0 ? 'var(--cy-accent)' : 'var(--cy-red)',
        }}>
          {rr !== '—' ? `${parseFloat(rr) >= 0 ? '' : ''}${rr}R` : '—'}
        </div>
      </div>
    </div>
  )
...
function StatMini({ val, label, color, sub }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '18px', color }}>
        {val}
      </div>
      <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '7px', fontWeight: 600, color: 'var(--cy-muted)', letterSpacing: '1.5px', textTransform: 'uppercase', marginTop: '2px' }}>
        {label}
      </div>
      <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '7px', color: 'var(--cy-muted)', marginTop: '1px' }}>
        {sub}
      </div>
    </div>
  )
...
function LogTradeModal({ onClose, onSuccess }) {
  const [form, setForm] = useState({
    ticker: '', account: '耀才', market: 'HK',
    entry_price: '', exit_price: '',
    stop_loss: '', quantity: '',
    commission: '', strategy_used: '', notes: '',
  })
  ...
  return (
    <Modal title="Log Closed Trade" onClose={onClose}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px' }}>
        <div><div style={labelStyle}>Ticker *</div>
          <input style={inputStyle} placeholder="e.g. NVDA" value={form.ticker} onChange={e => set('ticker', e.target.value)} /></div>
        ...
        <div><div style={labelStyle}>Entry Price *</div>
          <input style={inputStyle} type="number" placeholder="0.00" value={form.entry_price} onChange={e => set('entry_price', e.target.value)} /></div>
        <div><div style={labelStyle}>Exit Price *</div>
          <input style={inputStyle} type="number" placeholder="0.00" value={form.exit_price} onChange={e => set('exit_price', e.target.value)} /></div>
        <div><div style={labelStyle}>Stop Loss *</div>
          <input style={inputStyle} type="number" placeholder="0.00" value={form.stop_loss} onChange={e => set('stop_loss', e.target.value)} /></div>
        <div><div style={labelStyle}>Quantity *</div>
          <input style={inputStyle} type="number" placeholder="0" value={form.quantity} onChange={e => set('quantity', e.target.value)} /></div>
        <div><div style={labelStyle}>Commission</div>
          <input style={inputStyle} type="number" placeholder="0.00" value={form.commission} onChange={e => set('commission', e.target.value)} /></div>
        <div><div style={labelStyle}>Market</div>
          <select style={inputStyle} value={form.market} onChange={e => set('market', e.target.value)}>
            <option>HK</option><option>US</option>
          </select></div>
      </div>
      <div style={{ marginTop: '7px' }}>
        <div style={labelStyle}>Strategy Used</div>
        <input style={inputStyle} placeholder="e.g. S3 RSI Div + A1 OBV" value={form.strategy_used} onChange={e => set('strategy_used', e.target.value)} />
      </div>
      <div style={{ marginTop: '7px' }}>
        <div style={labelStyle}>Notes</div>
        <input style={inputStyle} placeholder="What worked / what didn't" value={form.notes} onChange={e => set('notes', e.target.value)} />
      </div>
